 ...
 */
namespace Dolphin\Override\Controller\Rewrite\Checkout\Cart;
...
class UpdateItemOptions extends \Magento\Checkout\Controller\Cart\UpdateItemOptions
{
    ...
    public function execute()
    {
        ...
        try {
			...
			$item->setData('generated_svg', $params['generated_svg']);
            if (is_string($item)) {
            ...
            if (!$this->_checkoutSession->getNoCartRedirect(true)) {
                if (!$this->cart->getQuote()->getHasError()) {
                    $message = __(
                        '%1 was added in your shopping cart.',
            ...
            $this->messageManager->addException($e, __('We can\'t added the item right now.'));
